...
void* BinarySemaphore_udp;	//二值信号量句柄  UDP数据接收信号量

struct udp_pcb *udppcb;  	//发送前测试 建立链接

u32 data_len_snmp;//用于接收的数据 测试snmp
//UDP 测试全局状态标记变量
//bit7:没有用到
...
//bit4~0:保留
 u8 udp_demo_flag;
 u8  Flag_test;//按键按下  建立UDP链接 标志位
 BaseType_t err;//任务通知值
 //任务句柄
TaskHandle_t Keyprocess_Handler;
void* LED1Task_Handler;
void* mainTask_Handler;
void* cameraTask_Handler;

TaskHandle_t macTask_Handler;
...
void udp_demo_set_remoteip(void)
{
 ...
 //	tbuf=pvPortMalloc(SRAMIN,100);	//申请内存
	tbuf=pvPortMalloc(100);	//申请内存
	if(tbuf==NULL)return;
	//前三个IP保持和DHCP得到的IP一致
...
void udp_demo_test(void)
{
	...
	u8 t=0; 
 	printf("udp test r\n");
	//xSemaphoreGiveFromISR(BinarySemaphore_udp, &xHigherPriorityTaskWoken2);//收到数据后向 snmp发送信号量
	...
	if(udppcb)//创建成功
	{ 
		printf("udp creat sucess\r\n");
		IP4_ADDR(&rmtipaddr,lwipdev.remoteip[0],lwipdev.remoteip[1],lwipdev.remoteip[2],lwipdev.remoteip[3]);
		err=udp_connect(udppcb,&rmtipaddr,161);//UDP客户端连接到指定IP地址和端口号的服务器
		if(err==ERR_OK)
		{
  ...
  UserSnmpDemo();//发送trap			
	...
	//udp_demo_connection_close(udppcb); //先不关
	//myfree(SRAMIN,tbuf);
} 
...
void udp_demo_recv(void *arg,struct udp_pcb *upcb,struct pbuf *p,struct ip_addr *addr,u16_t port)
{
	...
	if(p!=NULL)	//接收到不为空的数据时
	{
		...
		for(q=p;q!=NULL;q=q->next)  //遍历完整个pbuf链表
		{
			...
			data_len += q->len;  
     data_len_snmp=data_len;//udp接收的数据存下来			
