...
int SD_Rdy_Status =0x10;					// SD卡的状态，默认插入	

void* spi_receive_command;
void* spi_send_command;
u8 sd_init;//判断初始化有没有SD卡
...
//等待卡准备好
//返回值:0,准备好了;其他,错误代码
u8 SD_WaitReady(void)  //这个死等  改成SPI中断  有中断后发信号量
{
	u32 t=0;	 
	while(t<5000)
	{
	 ...
	 delay_ms(1);
	 /* 发送事件标志，表示任务正常运行 */
	// xEventGroupSetBits(xCreatedEventGroup, TASK_BIT_SD);	
	}
	return 1;
}
...
/*
 * 函数名：u8 SD_WaitReady_init(void)
 * 描述  ：初始化的时候用这个函数
 * 输入  ：无
 * 输出  ：无	
 */
//返回值:0,准备好了;其他,错误代码
u8 SD_WaitReady_init(void)
{
  u32 t=0;	 
	while(t<5000)
	{
