...
void SPI1_IRQHandler(void)
{
	...
	u8 recv[10];
	if(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) != RESET)  //发送完成
	{
    xSemaphoreGiveFromISR(spi_send_command,&xHigherPriorityTaskWoken);
  }
  if(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) != RESET)	//接收完成
	{
...
{		
	...
	while(xSemaphoreTake(spi_receive_command, portMAX_DELAY)!=pdTRUE)//检查指定的SPI标志位设置与否:接受缓存非空标志位
	{
		retry++;
		if(retry>200)
		return 0;	
