...
//  for(k=0;k<200;k++)
//	{	
//	 data_test[k]=pvPortMalloc(512);
//	 memset(data_test[k],0,512);
//	}
  ...
  {		
	 test_read(file3,buf_test[0],count);
	 //printf("strlen((char*)buf_test[0])=%d\n",strlen(buf_test[0]));

	 MD5Update(&context,(u8*)buf_test[0], 512);
	
	 
	 MD5Final(&context, digest);
   sprintf((char*)test_md5[count],"%s",digest);	
   memset(buf_test,0,520);		
	}
  sprintf((char*)test_md25[0],"%s%s",test_md5[0],test_md5[1]);		
	MD5Update(&context,test_md25[0], 32);
	MD5Final(&context, digest);
	f_close(file3);	
	vPortFree(file3);
 ...
 */
void md5_json(char* http_post_payload,unsigned short http_len)
{
  int len,j=0,i;
...
void md5_write_to_flash(void)
{
  u8 res,i,n4;		
	...
	//file_md5_boot=(FIL*)pvPortMalloc(1000);//申请内存
	file_md5_boot=(FIL*)pvPortMalloc(100);//申请内存
	{	
		res = f_open(file_md5_boot, "1:file_md5_boot.txt", FA_OPEN_ALWAYS | FA_WRITE);			
		if(res!=FR_OK)
		{ 
		 printf("open file_md5_boot error!\r\n");
		}
		res=f_lseek(file_md5_boot, file_md5_boot->fsize);			
		n4= f_write(file_md5_boot, buff,strlen(buff),&br);//http_post_payload_len-2
...
{ 
		u8 res,i,n4,res6;		
		//file3=(FIL*)pvPortMalloc(1000);//申请内存
		{	
			//res = f_open(file3, "1:post.bin", FA_OPEN_ALWAYS | FA_WRITE);			
			//if(res!=FR_OK)
//			{ 
//			 printf("open test3 error!\r\n");
//			}
			 
		 res6=f_lseek(file3, len2);//将文件在当前位置截断			
		 n4= f_truncate(file3);
//		 vPortFree(file3);
//		 f_close(file3);		
		}				
...
//void md5_test_bug(unsigned char digest[])
//{
//  u8 res,i,j=0;
//	int ret = 0;
//	FIL *file_md5;	
...
//	/* 计算文件MD5 */
//	file_md5=(FIL*)pvPortMalloc(1000);//申请内存
//	res= f_open(file_md5, "1:post.bin",  FA_READ|FA_OPEN_ALWAYS | FA_WRITE);		
//	if(res!=FR_OK)
//	{ 
//	 printf("open file_md5 error!\r\n");
//	}
//		
//	data=pvPortMalloc(file_md5->fsize);
//	if(!data)
//	{
//	}
//	printf("read before\n");
//	ret=f_read(file_md5, data, file_md5->fsize, &br);
//	printf("read over\n");
//	if(ret != st->fsize)
//	{
//		
//	} 
//	MD5Update(&context, data, file_md5->fsize);
//	f_close(file_md5);
//	vPortFree(data);
//  vPortFree(file_md5);	
//	
 ...
 * 输出  ：无	
 */ 
void test_read(FIL *file3,char buff[],u16 i)
{
	u8 ret,res;
	u16 bin_count,bin_remain;
	bin_count=(atoi(sub_bin_size))/512;	  //要再判断下能不能被512整除
	bin_remain=(atoi(sub_bin_size))%512;	
	if(i==bin_count)
	{
		f_read(file3, buff, bin_remain, &br);
	}
	else
	{
		res=f_lseek(file3, 512*i);
		{
		 ret=f_read(file3, buff, 512, &br); //res=f_lseek(file3, file3->fsize);
		}	
	}	
}
