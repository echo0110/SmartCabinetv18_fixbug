             ...
             TCHAR *Buffer, int BufferSize, const TCHAR *Filename)
{
	INI_FILETYPE *inirfp;
  int ok = 0;
	inirfp = (FIL*)pvPortMalloc(sizeof(FIL));
	
  if (Buffer == NULL || BufferSize <= 0 || Key == NULL)
    return 0;
  if (ini_openread(Filename, inirfp)) {
    ok = getkeystring(inirfp, Section, Key, -1, -1, Buffer, BufferSize);
    (void)ini_close(inirfp);
  } /* if */
  if (!ok)
...
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, const TCHAR *Filename)
{
  ...
  int len, match, flag, cachelen;
	...
	inirfp = (FIL*)pvPortMalloc(sizeof(FIL));
	iniwfp = (FIL*)pvPortMalloc(sizeof(FIL));
	
  assert(Filename != NULL);
  if (!ini_openread(Filename, inirfp)) {
    /* If the .ini file doesn't exist, make a new file */
    if (Key != NULL && Value != NULL) {
      if (!ini_openwrite(Filename, iniwfp))
        return 0;
      writesection(LocalBuffer, Section, iniwfp);
      writekey(LocalBuffer, Key, Value, iniwfp);
      (void)ini_close(iniwfp);
    } /* if */
    return 1;
  } /* if */
   ...
   */
  if (Key != NULL && Value != NULL) {
    (void)ini_tell(inirfp, &mark);
    match = getkeystring(inirfp, Section, Key, -1, -1, LocalBuffer, sizearray(LocalBuffer));
    if (match && _tcscmp(LocalBuffer,Value) == 0) {
      (void)ini_close(inirfp);
      return 1;
    } /* if */
    /* key not found, or different value -> proceed (but rewind the input file first) */
    (void)ini_seek(inirfp, &mark);
  } /* if */
  ...
  if (!ini_openwrite(LocalBuffer, iniwfp)) {
    (void)ini_close(inirfp);
    return 0;
  } /* if */
  (void)ini_tell(inirfp, &mark);
  ...
  if (len > 0) {
    do {
      if (!ini_read(LocalBuffer, INI_BUFFERSIZE, inirfp)) {
        /* Failed to find section, so add one to the end */
        flag = cache_flush(LocalBuffer, &cachelen, inirfp, iniwfp, &mark);
        if (Key!=NULL && Value!=NULL) {
          if (!flag)
            (void)ini_write(INI_LINETERM, iniwfp);  /* force a new line behind the last line of the INI file */
          writesection(LocalBuffer, Section, iniwfp);
          writekey(LocalBuffer, Key, Value, iniwfp);
        } /* if */
        return close_rename(inirfp, iniwfp, Filename, LocalBuffer);  /* clean up and rename */
      } /* if */
      ...
      if (!match || Key != NULL) {
        if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE)) {
          cache_flush(LocalBuffer, &cachelen, inirfp, iniwfp, &mark);
          (void)ini_read(LocalBuffer, INI_BUFFERSIZE, inirfp);
  ...
  for( ;; ) {
    ...
    if (Key == NULL) {
      (void)ini_tell(inirfp, &mark);  /* we are deleting the entire section, so update the read position */
    } else {
      if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE)) {
        cache_flush(LocalBuffer, &cachelen, inirfp, iniwfp, &mark);
        (void)ini_read(LocalBuffer, INI_BUFFERSIZE, inirfp);
  ...
  while (ini_read(LocalBuffer, INI_BUFFERSIZE, inirfp)) {
    if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE)) {
      cache_flush(LocalBuffer, &cachelen, inirfp, iniwfp, &mark);
      (void)ini_read(LocalBuffer, INI_BUFFERSIZE, inirfp);
      cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE);
