...
//#define ini_openread(filename,file)		(((file) = FileOpen((filename),FA_READ+FA_OPEN_EXISTING)) != NULL)
#define ini_openwrite(filename,file)  (f_open((file), (filename), FA_WRITE+FA_CREATE_ALWAYS) == FR_OK)
//#define ini_openwrite(filename,file)	(((file) = FileOpen((filename),FA_WRITE+FA_CREATE_ALWAYS)) != NULL)
#define ini_close(file)               (f_close(file) == FR_OK)
//#define ini_close(file)               	(FileClose(file) == FR_OK)
